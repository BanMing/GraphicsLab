...
{
	auto minmapTex = std::make_unique<Texture>();
	minmapTex->Name = "minmapTex";
	minmapTex->Filename = L"Textures/Exercise2.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), minmapTex->Filename.c_str(),
		minmapTex->Resource, minmapTex->UploadHeap));

	mTextures[minmapTex->Name] = std::move(minmapTex);
}
...
void CrateApp::BuildDescriptorHeaps()
{
	...
	auto minmapTex = mTextures["minmapTex"]->Resource;

	D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	srvDesc.Format = minmapTex->GetDesc().Format;
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MostDetailedMip = 2;
	srvDesc.Texture2D.MipLevels = minmapTex->GetDesc().MipLevels;
	srvDesc.Texture2D.ResourceMinLODClamp = 0.0f;

	md3dDevice->CreateShaderResourceView(minmapTex.Get(), &srvDesc, hDescriptor);
}
