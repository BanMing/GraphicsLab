...
void LitColumnsApp::LoadTextures()
{
	...
	auto headDiffTex = std::make_unique<Texture>();
	headDiffTex->Name = "headDiffTex";
	headDiffTex->Filename = L"../../Textures/head_diff.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), headDiffTex->Filename.c_str(),
		headDiffTex->Resource, headDiffTex->UploadHeap));

	mTextures[headDiffTex->Name] = std::move(headDiffTex);
...
void LitColumnsApp::BuildDescriptorHeaps() 
{
	...
	auto brickswoodCrateTexTex = mTextures["woodCrateTex"]->Resource;
	auto tileTex = mTextures["tileTex"]->Resource;
	auto headDiffTex = mTextures["headDiffTex"]->Resource;
	...
	srvDesc.Format = brickswoodCrateTexTex->GetDesc().Format;
	srvDesc.Texture2D.MipLevels = brickswoodCrateTexTex->GetDesc().MipLevels;
	md3dDevice->CreateShaderResourceView(brickswoodCrateTexTex.Get(), &srvDesc, hDescriptor);

	// offset to next descriptor in heap
	hDescriptor.Offset(1, mCbvSrvDescriptorSize);

	srvDesc.Format = headDiffTex->GetDesc().Format;
	srvDesc.Texture2D.MipLevels = headDiffTex->GetDesc().MipLevels;
	md3dDevice->CreateShaderResourceView(headDiffTex.Get(), &srvDesc, hDescriptor);
}
