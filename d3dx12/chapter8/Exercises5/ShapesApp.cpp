...
void ShapesApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	mLightPhi = MathHelper::Clamp(mLightPhi, 0.1f, XM_PIDIV2);
}
...
void ShapesApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	int i = 0;
	DirectX::XMFLOAT3 Strength1 = { 0.f, 1.f, 1.f };
	DirectX::XMFLOAT3 Strength2 = { 1.f, 1.f, 0.f };

	XMVECTOR lightDir = -MathHelper::SphericalToCartesian(1.f, mLightTheta, mLightPhi);

	while (i < 9)
	{
		bool flag = i % 2 == 0;
		mMainPassCB.Lights[i].Position = { (flag ? 1 : -1) *5.0f, 3.5f, -10.0f + (i % 5) * 5.0f };
		mMainPassCB.Lights[i].Strength = flag ? Strength1 : Strength2;
		mMainPassCB.Lights[i].SpotPower = 10;
		//mMainPassCB.Lights[i].Direction = { 0.f,-1.f,0.f };
...
void ShapesApp::BuildMaterials()
{
	...
	box->Name = "box";
	box->MatCBIndex = 0;
	box->DiffuseAlbedo = XMFLOAT4(Colors::WhiteSmoke);
	...
	grid->Name = "grid";
	grid->MatCBIndex = 1;
	grid->DiffuseAlbedo = XMFLOAT4(Colors::Gray);
	grid->FresnelR0 = XMFLOAT3(0.01f, 0.01f, 0.01f);
	grid->Roughness = 0.125f;

	auto sphere = std::make_unique<Material>();
	sphere->Name = "sphere";
	sphere->MatCBIndex = 2;
	sphere->DiffuseAlbedo = XMFLOAT4(Colors::Green);
